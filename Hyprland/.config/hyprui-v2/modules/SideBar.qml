    ...
    PanelWindow {
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                Item {
                    Layout.fillWidth:    true
                    implicitHeight:      wpBtn.btnSize

                    Rectangle {
                        id: wpBtn
                        readonly property int btnSize: Math.round(UI.panelThickness * 0.70)
                        anchors.horizontalCenter: parent.horizontalCenter
                        width: btnSize; height: btnSize; radius: btnSize / 2
                        color: UI.wallpaperPickerVisible ? HyprUITheme.secondary : HyprUITheme.active.surface
                        Behavior on color { ColorAnimation { duration: 150 } }
                        Text {
                            anchors.centerIn: parent
                            text: "󰸉"; font.family: "MesloLGS NF"; font.pixelSize: UI.fontSize.md
                            color: UI.wallpaperPickerVisible ? HyprUITheme.active.background : HyprUITheme.active.text
                        }
                        MouseArea {
                            anchors.fill: parent; hoverEnabled: true
                            onClicked: UI.toggleWallpaperPicker()
                            onEntered: wpTip.requestShow()
                            onExited:  wpTip.requestHide()
                        }
                        Tooltip { id: wpTip; text: "Wallpapers"; orientation: "right"; parent: win.contentItem; target: wpBtn }
                    }
                }
