...
Item {
    ...
    // Unified hue for Performance Profile (Redmi: silent/balanced/performance/turbo)
    readonly property color profileStart: {
        if (powerProfile === "turbo") return window.red;
        if (powerProfile === "performance") return window.peach;
        if (powerProfile === "silent") return window.green;
        return window.blue; // balanced
    }
    ...
    Process {
        ...
        command: ["bash", "-c", 
            ...
            "idx=$(cat /run/power-mod-state 2>/dev/null); case $idx in 0) echo silent;; 1) echo balanced;; 2) echo performance;; 3) echo turbo;; *) echo balanced;; esac; " +
            ...
            "cat /run/power-mod-charge-state 2>/dev/null || echo 'off'"
        ]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (lines.length >= 6) {
                    if (lines.length >= 7) {
                        window.chargeLimitEnabled = (lines[6].trim() === "on");
                    }
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Repeater {
                        model: ListModel {
                            ListElement { cmd: "bash $HOME/.config/hyprui-v2/popups/lock.sh"; icon: ""; baseColor: "mauve"; weight: 1.0 }
                            ListElement { cmd: "bash $HOME/.config/hyprui-v2/popups/lock.sh & systemctl suspend"; icon: "ᶻ 𝗓 𐰁"; baseColor: "blue"; weight: 1.0 }
                ...
                // 3. POWER PROFILES DOCK (Redmi: silent/balanced/performance/turbo)
                Rectangle {
                    Layout.fillWidth: true
                    ...
                    Rectangle {
                        ...
                        radius: 10
                        x: {
                            if (window.powerProfile === "silent")      return 1;
                            if (window.powerProfile === "balanced")     return width + 1;
                            if (window.powerProfile === "performance")  return (width * 2) + 1;
                            return (width * 3) + 1; // turbo
                        }
                        ...
                        Repeater {
                            ...
                            delegate: Item {
                                ...
                                RowLayout {
                                    ...
                                    spacing: 6
                                    Text {
                                        font.family: "Iosevka Nerd Font"; font.pixelSize: 17
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        ...
                                        let idx = name === "silent" ? "0" : name === "balanced" ? "1" : name === "performance" ? "2" : "3";
                                        // Privileged: setuid-root, zero password prompt
                                        Quickshell.execDetached(["/usr/local/bin/power-mod-helper", "set", idx]);
                                        // Unprivileged: apply display refresh via hyprctl
                                        // Silent=60Hz, Balanced/Performance=165Hz, Turbo=max available
                                        Quickshell.execDetached(["bash", "-c",
                                            ...
                                            "if [ \"$IDX\" = \"0\" ]; then " +
                                                "HZ=60; " +
                                            "elif [ \"$IDX\" = \"3\" ]; then " +
                                                "HZ=$(echo \"$MON\" | jq -r '.availableModes[]' | awk -F@ '{v=$2; gsub(/[^0-9.]/, \"\", v); print int(v)}' | sort -n | tail -1); " +
                                                "[ -z \"$HZ\" ] && HZ=165; " +
                                            "else HZ=165; fi; " +
                                            "hyprctl keyword monitor \"${NAME},${W}x${H}@${HZ},${PX}x${PY},${SCALE}\"",
                                            "_", idx
                                        ]);
                                    }
                                }
                ...
                // 4. CHARGE LIMIT TOGGLE (80% BMS conservation mode)
                Rectangle {
                    id: chargeLimitRow
                    Layout.fillWidth: true
                    Layout.preferredHeight: 48
                    radius: 14
                    color: chargeMa.containsMouse ? "#18ffffff" : "#0dffffff"
                    border.color: "#1affffff"
                    border.width: 1

                    opacity: introProfiles
                    transform: Translate { y: 24 * (1.0 - introProfiles) }
                    ...
                    RowLayout {
                        ...
                        Text {
                            font.family: "Iosevka Nerd Font"; font.pixelSize: 18
                            color: window.chargeLimitEnabled ? window.green : window.subtext0
                            text: window.chargeLimitEnabled ? "󰂄" : "󰂒"
                            Behavior on color { ColorAnimation { duration: 200 } }
                        }
                        ...
                        Text {
                            ...
                            color: window.chargeLimitEnabled ? window.green : window.subtext0
                            text: window.chargeLimitEnabled ? "80% ON" : "100% OFF"
                            Behavior on color { ColorAnimation { duration: 200 } }
                        }
                        ...
                        Rectangle {
                            ...
                            color: window.chargeLimitEnabled ? window.green : window.surface1
                            Behavior on color { ColorAnimation { duration: 200 } }

                            Rectangle {
                                width: 16; height: 16; radius: 8
                                anchors.verticalCenter: parent.verticalCenter
                                x: window.chargeLimitEnabled ? parent.width - width - 3 : 3
                                color: "white"
                                Behavior on x { NumberAnimation { duration: 200; easing.type: Easing.OutBack; easing.overshoot: 1.5 } }
                            }
                        }
                    }

                    // MouseArea as sibling of RowLayout — avoids "anchors in layout" warning
                    MouseArea {
                        id: chargeMa
                        anchors.fill: parent
                        hoverEnabled: true
                        cursorShape: Qt.PointingHandCursor
                        onClicked: {
                            let newState = window.chargeLimitEnabled ? "off" : "on";
                            window.chargeLimitEnabled = !window.chargeLimitEnabled;
                            // power-mod-helper is setuid-root — no password prompt
                            Quickshell.execDetached(["/usr/local/bin/power-mod-helper", "charge", newState]);
                        }
                    }
                }
            }
