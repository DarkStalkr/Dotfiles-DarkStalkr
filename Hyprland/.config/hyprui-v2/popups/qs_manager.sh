...
SRC_DIR="/home/sohighman/Imágenes/Wallpapers"
...
# Shorthand: bare widget name → "toggle <widget>"
# e.g. qs_manager.sh wallpaper  ≡  qs_manager.sh toggle wallpaper
if [[ -n "$ACTION" && "$ACTION" != "close" && "$ACTION" != "open" && "$ACTION" != "toggle" && ! "$ACTION" =~ ^[0-9]+$ ]]; then
    TARGET="$ACTION"
    ACTION="toggle"
fi
...
# -----------------------------------------------------------------------------
# VERIFY HYPRUI SHELL IS RUNNING
# -----------------------------------------------------------------------------
QS_PID=$(pgrep -f "quickshell.*shell\.qml")
if [[ -z "$QS_PID" ]]; then
    echo "Warning: HyprUI shell (shell.qml) is not running" >&2
    exit 1
fi
