...
Item {
    ...
    readonly property string srcDir: "/home/sohighman/Imágenes/Wallpapers"
            ...
            function pickWallpaper() {
                ...
                if (window.currentFilter === "Search" && window.hasSearched) {
                    ...
                    if (alreadyExists) {
                        const applyScript = `
                            ...
                            export TEMPLATE_PATH="$HOME/.config/hyprui-v2/popups/colors.json.template"
                            
                            (
                                pkill swaybg || true
                                pkill mpvpaper || true
                                swaybg -i "$DEST_FILE" -m fill &
                                cp "$DEST_FILE" /tmp/lock_bg.png
                                
                                if command -v matugen &> /dev/null; then
                                    matugen image "$FINAL_THUMB"
                                    matugen template "$TEMPLATE_PATH" "/tmp/qs_colors.json"
                                    bash "$RELOAD_SCRIPT"
                                fi
                            ) >/dev/null 2>&1 & disown
                        `;
                ...
                const fullScript = `
                    ...
                    (
                        pkill swaybg || true
                        pkill mpvpaper || true
                        if [[ "$WALL_FILE" =~ \\.(mp4|mkv|mov|webm)$ ]]; then
                            mpvpaper -o 'loop --no-audio --hwdec=auto' '*' "$WALL_FILE" &
                            cp "$THUMB_FILE" /tmp/lock_bg.png
                        else
                            swaybg -i "$WALL_FILE" -m fill &
                            cp "$WALL_FILE" /tmp/lock_bg.png
                        fi
                        
                        if command -v matugen &> /dev/null; then
                            matugen image "$THUMB_FILE"
                            matugen template "$TEMPLATE_PATH" "/tmp/qs_colors.json"
                            bash "$RELOAD_SCRIPT"
                        fi
                    ) >/dev/null 2>&1 & disown
