...
Item {
    ...
    function applyPresetOptimistically(presetName) {
        ...
        if (presets[presetName]) {
            ...
            execCmd(`${Quickshell.env("HOME")}/.config/hyprui-v2/popups/music/equalizer.sh preset ${presetName}`);
        }
    }
    ...
    Process {
        id: musicProc
        running: true
        command: ["bash", "-c", `${Quickshell.env("HOME")}/.config/hyprui-v2/popups/music/music_info.sh`]
    ...
    Process {
        id: eqProc
        running: true
        command: ["bash", "-c", `${Quickshell.env("HOME")}/.config/hyprui-v2/popups/music/equalizer.sh get`]
        stdout: StdioCollector {
            onStreamFinished: {
                if (this.text) {
                            ...
                            Slider {
                                ...
                                onPressedChanged: {
                                    if (pressed) {
                                        ...
                                        root.execCmd(`${Quickshell.env("HOME")}/.config/hyprui-v2/popups/music/player_control.sh seek ${value.toFixed(2)} ${root.musicData.length} "${safePlayer}"`);
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    if (root.eqData.pending) {
                                        ...
                                        root.triggerEqLightning();
                                        root.execCmd(`${Quickshell.env("HOME")}/.config/hyprui-v2/popups/music/equalizer.sh apply`);
                                    }
                                }
                            }
                        }
                                        ...
                                        Slider {
                                            ...
                                            onPressedChanged: {
                                                if (!pressed) {
                                                    ...
                                                    root.execCmd(`${Quickshell.env("HOME")}/.config/hyprui-v2/popups/music/equalizer.sh set_band ${modelData.idx} ${Math.round(value)}`);
                                                }
                                            }
